...
The agents collect historical liquidity pool data by making [subgraph](https://thegraph.com) queries. They preprocess the data and train a prediction algorithm. Once enough data is collected and the training process has finished, the agents transit to an estimation stage where they periodically compute the predictions based on the trained algorithm. Next, the predictions are signed and a keeper agent is chosen to send the signed predictions. Note that the agents reach consensus on all the steps involving the collection, preprocessing, training and prediction of the data to be sent.
...
1. Fetch the ML APY Prediction Oracle service.
	...
	```bash
	autonomy fetch valory/apy_estimation:0.1.0:bafybeibi4c4iiqmrwgsl5lw5oibz22xekvheqllmun7s2clbzuqivpjpxi --service
	```
